...
<div class="rfc-header">
  ...
  <span class="project-tag" style="background:#fef3c7;color:#92400e;border-color:#d97706;">Not adopted, built in full</span>
...
> **Status.** This redesign was not adopted by the platform. I implemented it in full regardless. The four defence layers below exist as working code, not as a paper proposal. The numbers in *Design targets* are the criteria the implementation was built against; they have not been validated under a reproduced production load, which is the honest limit of what I can claim.
...
**A single global worker pool.** Fewer workers and a simpler topology, but one regional outage could exhaust the pool and take everything down with it. Rejected for insufficient isolation.
...
The implementation was built against these criteria. They are targets it is designed to meet, not measurements taken under reproduced production load.
...
I'd have insisted on production-load simulation before the design review rather than after. The architecture is a bet about behaviour under a load nobody has reproduced, and the pause-granularity tradeoff is exactly the kind of thing a real load test either vindicates or embarrasses. Building it out fully closed the implementation risk but not that one, the load test remains the next gate, and it's the gap I'd close first.
